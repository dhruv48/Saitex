...
public partial class Module_Sewing_Thread_Pages_GetSODisTex : System.Web.UI.Page
{
            ...
            if (!IsPostBack)
            {
                ...
                if (Session["dtDicRate"] != null)
                {
                    ...
                    foreach (DataRow dr in dtSORate1.Rows)
                    {
                        ...
                        if (dr["YARN_CODE"].ToString() == YARN_CODE && dr["SHADE_CODE"].ToString() == SHADE_CODE)
                        {
                            double dAmount = 0;
    ...
    private void CreateDataTable()
    {
        try
        {
            ...
            dtSORate1.Columns.Add("SHADE_CODE", typeof(string));
        ...
        try
        {
            foreach (GridViewRow Row in grdRate.Rows)
            {
                ...
                foreach (DataRow dr in dtSORate1.Rows)
                {
                    ...
                    if (sCode == Code && sITemCode == YARN_CODE && SHADE_CODE == sSHADE_CODE)
                    {
    ...
    private void AddRowInDataTable()
    {
        try
        {
            ...
            if (dtSORate1.Rows.Count > 0)
            {
                ...
                dvRate.RowFilter = "YARN_CODE='" + YARN_CODE + "' and SHADE_CODE='" + SHADE_CODE + "' and COMPO_CODE='" + Code + "' and Uniqueid <> " + Uniqueid;
            ...
            if (!IsDuplicate)
            {
                ...
                if (dvRateComponent.Count > 0)
                {
                    ...
                    if (rate > 0)
                    {
                        ...
                        {
                            DataView dvv = new DataView(dtSORate1);
                            dvv.RowFilter = "YARN_CODE='" + YARN_CODE + "' and SHADE_CODE='" + SHADE_CODE + "' and COMPO_CODE='" + ddlBaseComponent.SelectedItem.Text.Trim() + "'";
    ...
    private void CalculateFinalAmount()
    {
        try
        {
            if (dtSORate1 != null && dtSORate1.Rows.Count > 0)
            {
                ...
                dvRate.RowFilter = "YARN_CODE='" + YARN_CODE + "' and SHADE_CODE='" + SHADE_CODE + "'";
                if (dvRate.Count > 0)
                {
