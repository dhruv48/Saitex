...
public partial class Module_Waste_Controls_WasteIssue : System.Web.UI.UserControl
{
    ...
    private void gridmaterialledger()
    {
        ...
        try
        {
            ...
            DataTable dt = new DataTable();
            dt = SaitexBL.Interface.Method.TX_WASTE_MASTER.MaterialTransaction(BRANCH_CODE, DEPT_CODE, ITEM_CATE, ITEM_TYPE, ITEM_CODE, StDate, EnDate, TRAN_TYPE, PARTY_CODE,YEAR);
            if (dt.Rows.Count > 0)
            {
    ...
    protected void imgbtnClear_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            Response.Redirect("./WasteIssue.aspx", false);
        }
        catch (Exception ex)
        {
            errorLog.ErrHandler.LogError(ex, @"Problem in " + ex.TargetSite.Name + "{0}\n Message: " + ex.Message);
    ...
    protected void imgbtnPrint_Click(object sender, ImageClickEventArgs e)
    {
        ...
        try
        {
            ...
            string URL = "../Report/WasteIssueQueryReport.aspx";
    ...
    private void bindddltrntype()
    {
        ...
        try
        {
            ...
            DataTable dt = SaitexBL.Interface.Method.TX_WASTE_IR_MST.getTransType();
            if (dt != null && dt.Rows.Count > 0)
            {
    ...
    protected DataTable GetItems(string text, int startOffset)
    {
        try
        {
            ...
            if (startOffset != 0)
            {
                whereClause += "  AND ITEM_code NOT IN (SELECT ITEM_CODE FROM (SELECT ITEM_CODE, ITEM_DESC, CAT_CODE||'@'|| ITEM_DESC||'@'||UOM as Combined   FROM TX_WASTE_MST WHERE ITEM_CODE LIKE :SearchQuery OR ITEM_TYPE LIKE :SearchQuery OR ITEM_DESC LIKE :SearchQuery ORDER BY ITEM_CODE) asd WHERE ROWNUM <= " + startOffset + ")  ";
            }
